...
#ifdef USE_NUMERIC_SEARCH
#define ACTIVE_RPG_BUILDER NumericRPGBuilder
#define ACTIVE_SOLVER NumericFF
#else
#define ACTIVE_RPG_BUILDER RPGBuilder
#define ACTIVE_SOLVER FF
#endif
...
int main(int argc, char * argv[])
{
    ...
    if (Globals::optimiseSolutionQuality && Globals::givenSolutionQualityDefined) {
        if (ACTIVE_RPG_BUILDER::getMetric()) {
            ...
            ACTIVE_RPG_BUILDER::getHeuristic()->metricHasChanged();
        }
    }
    ...
    if (readInAPlan) {
        ...
        reachesGoals = true;
#ifndef NDEBUG
        spSoln = ACTIVE_SOLVER::doBenchmark(reachesGoals, spSoln, false);
#endif
    } else {
        planAndConstraints = ACTIVE_SOLVER::search(reachesGoals);
    }
    ...
    if (spSoln) {
        ...
        for (int pass = 0; pass < 2; ++pass) {
            if (pass) {
                ...
                if (!spSoln->empty()) {
                    ...
                    spSoln = ACTIVE_SOLVER::reprocessPlan(spSoln, planAndConstraints.constraints);
        ...
        if (benchmark) {
            ACTIVE_SOLVER::doBenchmark(reachesGoals, spSoln);
        }
